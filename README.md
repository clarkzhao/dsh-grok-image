...
DeepSeek Harness 插件:把 **Grok Imagine 图片生成**(订阅额度)注册成 DSH 的模型工具 `image_gen`。针对 **DSH 0.1.5-rc.2**。
...
- 凭据不会自动续期。自动把 grok CLI token 挂进 DSH：见 [dsh-llm-grok/docs/credential-sync.md](https://github.com/clarkzhao/dsh-llm-grok/blob/main/docs/credential-sync.md)
...
本插件**不**自建 HTTP 路由。装了 `dsh-airp` 时，生成图挂到 Host 的 `airpStage`（`/airp-media`）。工具回执带同源绝对 URL，叙述者写进对白：
